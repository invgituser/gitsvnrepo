...
public class UserLogonDAOImpl implements UserLogonDAO {
	...
	public List<UserLogon> getInvUserList(){
		...
		try {
		SQLQuery query = sessionFactory.getCurrentSession().createSQLQuery("select logonid, userid, email from user_logon");
	...
	public UserLogon getInvUserDetails(Long id){
		UserLogon ul=null;
		try {
		SQLQuery query = sessionFactory.getCurrentSession().createSQLQuery("select logonid, userid, email from user_logon where logonid= "+id);
		List<Object[]> rows = query.list();
